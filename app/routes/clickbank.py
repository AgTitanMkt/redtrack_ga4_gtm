"""
Webhook ClickBank — Recebe IPN (Instant Payment Notification).
...
@router.get("/clickbank")
@router.post("/clickbank")
async def webhook(request: Request):
    logger.info("═══ CLICKBANK WEBHOOK ═══")
    ...
    # ── Extrai campos ────────────────────────────────────────────
    click_id = params.get("clickid") or data.get("clickid")
    payout = float(data.get("payout", params.get("payout", 0)))
    order_id = data.get("order_id") or params.get("order_id") or data.get("receipt", "cb-unknown")

    event_id = f"purchase_{order_id}"
    dedupe_key = f"clickbank:{order_id}"

    logger.info("ClickBank | order=%s | click=%s | payout=%.2f", order_id, click_id, payout)

    # ── Salva conversão ──────────────────────────────────────────
    ok = save_conversion("clickbank", event_id, order_id, click_id, payout, data or params)

    if not ok:
        logger.warning("ClickBank: save falhou (duplicado?) order=%s", order_id)
    ...
    if ok and click_id:
        ...
        logger.info("ClickBank: postback enfileirado para click=%s", click_id)
    elif not click_id:
        logger.info("ClickBank: sem click_id, pulando RedTrack")

    # ── GA4 + GTM SS ─────────────────────────────────────────────
    _send_ga4_purchase(click_id, order_id, event_id, dedupe_key, payout)

    return {"ok": True}


def _send_ga4_purchase(click_id, order_id, event_id, dedupe_key, payout):
    ...
    try:
        ...
        ga4_params = {
            ...
            "value": payout,
            "currency": "USD",
            "items": [{"item_id": str(order_id), "item_name": "ClickBank Purchase", "price": payout, "quantity": 1}],
        }
